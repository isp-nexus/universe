...
import { isUserFile } from "./program.js"

const logger = ConsoleLogger.withPrefix("Schema Generator")
const REGEX_IMPORT_WITH_ASSERTION = /,\s*\{.*\}/
...
const DEFAULT_GENERATOR_OPTIONS = {
	...
	constAsEnum: false,
} as const satisfies GeneratorOptions
...
type MetaDefinitionFields = "ignore" | "internal"
...
/**
 * Given a Symbol, finds the place it was declared and chases parent pointers until we find a node
 * where SyntaxKind === SourceFile.
 */
 ...
 *
 * Many of these validation keywords are defined here:
...
/**
 * Subset of descriptive, non-type keywords that are permitted alongside a $ref. Prior to JSON
 * Schema draft 2019-09, $ref is a special keyword that doesn't permit keywords alongside it, and so
 * AJV may raise warnings if it encounters any type-related keywords; see
 * https://github.com/ajv-validator/ajv/issues/1121
 */
...
export class JsonSchemaGenerator {
	/**
	 * This map holds references to all reffed definitions, including schema overrides and generated
	 * definitions.
	 */
	private reffedDefinitions = new Map<string, Definition>()

	/**
	 * This map only holds explicit schema overrides. This helps differentiate between user defined
	 * schema overrides and generated definitions.
	 */
	private schemaOverrides = new Map<string, Definition>()

	/**
	 * If true, this makes constants be defined as enums with a single value. This is useful for cases
	 * where constant values are not supported, such as OpenAPI.
	 */
	private constAsEnum: boolean

	/**
	 * Types are assigned names which are looked up by their IDs. This is the map from type IDs to
	 * type names.
	 */
	private typeNamesById: { [id: number]: string } = {}
	/**
	 * Whenever a type is assigned its name, its entry in this dictionary is set, so that we don't
	 * give the same name to two separate types.
	 */
	private typeIdsByName: { [name: string]: number } = {}

	constructor(
		/**
		 * All types for declarations of classes, interfaces, enums, and type aliases defined in all TS
		 * files.
		 */
		public allSymbols: Map<string, ts.Type>,
		/**
		 * All symbols for declarations of classes, interfaces, enums, and type aliases defined in
		 * non-default-lib TS files.
		 */
		public userSymbols: Map<string, ts.Symbol>,
		/**
		 * Maps from the names of base types to the names of the types that inherit from them.
		 */
		public inheritingTypes: Map<string, string[]>,
		public tc: ts.TypeChecker,
		public options: GeneratorOptions
	) {
		this.constAsEnum = options.constAsEnum
	}
	...
	private resetSchemaSpecificProperties(includeAllOverrides: boolean = false) {
		...
		if (includeAllOverrides) {
			this.schemaOverrides.forEach((value, key) => {
				this.reffedDefinitions.set(key, value)
			})
		}
	}
	...
	private parseCommentsIntoDefinition(
		...
		jsdocs.forEach((doc) => {
			...
			if (name.startsWith("TJS-")) {
				...
				const match: string[] | RegExpExecArray | null = new RegExp(REGEX_TJS_JSDOC).exec(originalText)
			...
			if (name.includes(".")) {
				...
				if (parts.length === 2 && subDefinitions[key]) {
					;(definition as DefinitionIndex)[key] = {
						...
						[parts[1]!]: text ? parseValue(text) : true,
					}
				}
			}

			if (validationKeywords[name as keyof typeof validationKeywords]) {
				;(definition as DefinitionIndex)[name] = text === undefined ? "" : parseValue(text)
			} else {
				// special annotations
	...
	private getDefinitionForRootType(
		...
		definition: Definition,
		defaultNumberType = this.options.defaultNumberType,
		...
		if (tupleType) {
			...
			} else if (propertyTypeString === "Set") {
				definition.type = "array"
				...
				if (value !== undefined) {
					...
					switch (typeofValue) {
						...
						case "boolean":
							...
							break
						case "number":
							definition.type = this.options.defaultNumberType
							break
						case "object":
							definition.type = "null"
							break
					...
					if (
						...
						const foo = Array.from((<any>propertyType).members).find((member: any) => {
							return member[0]! !== "__index"
						})
						...
						definition.type = "string"
						// @ts-ignore TS doesn't know about templateFlags
	...
	private getDefinitionForProperty(prop: ts.Symbol, node: ts.Node): Definition | null {
		...
		if ("internal" in definition) return null
		if ("ignore" in definition) return null
		...
		if (valDecl?.initializer) {
			...
			if ((<any>initial).expression) {
				// node
				logger.warn("Initializer is expression for property " + propertyName)
				...
				try {
					...
					if (
						...
						logger.warn("Unknown initializer for property " + propertyName + ": " + val)
					}
				} catch (_error) {
					logger.warn("Exception evaluating initializer for property " + propertyName)
				}
			}
		}
				...
				if (initial) {
					if ((<any>initial).expression) {
						...
						if (text) {
							...
							logger.warn("Initializer is expression for enum: " + fullName + "." + caseLabel)
						}
		...
		for (const valueType of unionType.types) {
			...
			if (value !== undefined) {
				...
				if (keys.length === 1 && keys[0] === "type") {
					if (typeof def.type !== "string") {
						logger.error("Expected only a simple type.")
					} else {
	...
	private getClassDefinition(clazzType: ts.Type, definition: Definition): Definition {
		...
		if (this.options.typeOfKeyword && node.kind === ts.SyntaxKind.FunctionType) {
		...
		if (modifierFlags & ts.ModifierFlags.Abstract && this.inheritingTypes.has(fullName)) {
			const oneOf = this.inheritingTypes.get(fullName)!.map((typename) => {
				return this.getTypeDefinition(this.allSymbols.get(typename)!)
			})
			...
			if (this.options.defaultProps) {
				definition.defaultProperties = []
			}
			if (this.options.noExtraProps && definition.additionalProperties === undefined) {
				definition.additionalProperties = false
			}
			...
			if (this.options.required) {
				...
				if (requiredProps.length > 0) {
					definition.required = unique(requiredProps)
				}
			}
		}
		return definition
	...
	private getTypeName(typ: ts.Type): string {
		...
		const existingName = this.typeNamesById[id]
	...
	private makeTypeNameUnique(_typ: ts.Type, baseName: string): string {
		// const id = (typ as any).id as number
	...
	private getTypeDefinition(
		...
		if (prop) {
			...
			if (defs.hasOwnProperty("internal") || defs.hasOwnProperty("ignore") || defs.hasOwnProperty("type")) {
				return defs
			}
		}
		...
		let isRawType = false
		const fqn = symbol ? this.tc.getFullyQualifiedName(symbol) : null
		...
		if (!symbol) {
			...
			if (fqn !== "Window") {
				const indexInfo = this.tc.getIndexInfoOfType(typ, ts.IndexKind.Number)

				if (fqn === "Date" || symbol.name === "integer" || indexInfo !== undefined) {
					isRawType = true
				}
			}
		}
		...
		const asTypeAliasRef = asRef && reffedType && (this.options.aliasRef || isStringEnum)
		if (!asTypeAliasRef) {
			if (
				isRawType ||
		...
		if (asTypeAliasRef) {
			...
			if (this.options.uniqueNames && typ.symbol) {
		...
		if (fullTypeName === "Set") {
			const [typeArgument] = this.tc.getTypeArguments(typ as ts.TypeReference)

			if (typeArgument && typeArgument.symbol) {
				const typeSymbol = typeArgument.symbol
				fullTypeName = typeSymbol.escapedName as string
				isRawType = true
				const typeArgumentDefinition = this.getTypeDefinition(typeArgument, true, unionModifier, typeSymbol, typeSymbol)

				definition.type = "array"
				definition.items = typeArgumentDefinition
		...
		if (asRef) {
			const possibleOverride = this.schemaOverrides.get(fullTypeName)

			if (possibleOverride) {
				returnedDefinition = {
					$ref: possibleOverride.$ref,
				}
			} else {
		...
		if (overrideDefinition) {
			// this.reffedDefinitions.set(fullTypeName, overrideDefinition)
		} else if (!asRef || !this.reffedDefinitions.has(fullTypeName)) {
			if (asRef) {
				...
				this.reffedDefinitions.set(fullTypeName, reffedDefinition)

				if (this.options.titles && fullTypeName) {
					definition.title = fullTypeName
				}
			}

			const node = symbol?.getDeclarations()?.[0] || null
		...
		if (this.recursiveTypeRef.get(fullTypeName) === definition) {
			...
			if (this.reffedDefinitions.has(fullTypeName)) {
	...
	public setSchemaOverride(symbolName: string, schema: Definition): void {
		if (this.schemaOverrides.has(symbolName)) return

		ConsoleLogger.info(`Overriding schema for ${symbolName}`)
	...
	public getSchemaForSymbol(
		...
		if (!this.allSymbols.get(symbolName) && !overrideDefinition) {
		...
		if (overrideDefinition) {
			...
			def = this.getTypeDefinition(
				this.allSymbols.get(symbolName)!,
				this.options.topRef,
				undefined,
				undefined,
				undefined,
				this.userSymbols.get(symbolName)
			)
		}

		if (this.options.ref && includeReffedDefinitions && this.reffedDefinitions.size > 0) {
			def.definitions = Object.fromEntries(this.reffedDefinitions)
		}
	...
	public getSchemaForSymbols(
		...
		if (this.options.ref && includeReffedDefinitions && Object.keys(this.reffedDefinitions).length > 0) {
			root.definitions = {
				...root.definitions,
				...Object.fromEntries(this.reffedDefinitions),
			}
		}
...
export function buildGenerator(
	...
	const allSymbols = new Map<string, ts.Type>()
	const userSymbols = new Map<string, ts.Symbol>()

	const inheritingTypes = new Map<string, string[]>()
	...
	program.getSourceFiles().forEach((sourceFile, _sourceFileIdx) => {
		...
		function inspect(node: ts.Node) {
			if (
				...
				const name = !generatorOptions.uniqueNames ? typeName : `${typeName}.${generateHashOfNode(node, relativePath)}`

				symbols.push({ name, typeName, fullyQualifiedName, symbol })

				if (!userSymbols.has(name)) {
					allSymbols.set(name, nodeType)
				}

				if (isUserFile(sourceFile)) {
					userSymbols.set(name, symbol)
				}
				...
				baseTypes.forEach((baseType) => {
					...
					inheritingTypes.set(baseName, [...(inheritingTypes.get(baseName) || []), name])
				})
			} else {
	...
	return new JsonSchemaGenerator(allSymbols, userSymbols, inheritingTypes, tc, {
